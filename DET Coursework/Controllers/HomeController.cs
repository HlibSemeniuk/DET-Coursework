...
namespace DET_Coursework.Controllers
{
    public class HomeController : Controller
    {
        ...
        private async Task<List<PublicationInfo>> GetAllPublicationsAsync()
        {
            ...
            SELECT ?назва ?типНазва ?дата ?сторінок ?авторів ?сторінокНаАвтора ?мова
                   (GROUP_CONCAT(DISTINCT ?авторПІБ; separator="", "") AS ?Автори)
                   ?журналНазва
                   (GROUP_CONCAT(DISTINCT ?галузьНазва; separator="", "") AS ?Галузі)
                   (GROUP_CONCAT(DISTINCT ?ключовеСлово; separator="", "") AS ?Ключові_слова)
            WHERE {
              ?публікація rdf:type ?тип .
              ?тип rdfs:subClassOf* onto:Наукова_Публікація .
              FILTER (?тип != onto:Наукова_Публікація)

              OPTIONAL { ?публікація onto:назва ?назва . }
              OPTIONAL { ?публікація onto:дата_публікації ?дата . }
              OPTIONAL { ?публікація onto:кількість_сторінок ?сторінок . }
              OPTIONAL { ?публікація onto:кількість_авторів ?авторів . }
              OPTIONAL { ?публікація onto:кількість_сторінок_на_автора ?сторінокНаАвтора . }
              OPTIONAL { ?публікація onto:мова ?мова . }

              OPTIONAL { 
                ?публікація onto:маєАвтора ?автор .
                ?автор onto:ПІБ ?авторПІБ .
              }

              OPTIONAL {
                ?публікація onto:опубліковано_в ?журнал .
                ?журнал onto:назва ?журналНазва .
              }

              OPTIONAL {
                ?публікація onto:належить_до_галузі_знань ?галузь .
                ?галузь onto:назва ?галузьНазва .
              }

              OPTIONAL {
                ?публікація onto:містить_ключове_слово ?ключове .
                ?ключове onto:назва ?ключовеСлово .
              }

              BIND(STRAFTER(STR(?тип), ""#"") AS ?типНазва)
            }
